...
class RandomVariable(IRandom):
    ...
    def characteristic_function(self, t: np.complex64) -> np.complex64:
        try:
            res = quad(
                lambda x: np.exp(1j * t * x) * self.pdf(x),
                self._interval_a,
                self._interval_b,
            )[0]
            return res
    ...
    def inverse_cdf(self, x: np.float64) -> np.float64:
        return root_scalar(
            lambda t: self.cdf(t) - x, bracket=(self._interval_a, self._interval_b)
        )[0]
    ...
    def variance(self) -> np.float64:
        try:
            res = quad(
                lambda x: x * x * self.pdf(x), self._interval_a, self._interval_b
            )[0]
            res -= self.mean() ** 2
            return res
        except Exception as e:
    ...
    def precompute_cdf_table_linear(self, granularity=100, x0=None):
        xs = np.linspace(
            self._interval_a, self._interval_b, granularity, endpoint=True
        )[1:-1]
    ...
    def precompute_cdf_table(
        self, granularity=100, precompute_table_approximation="linear", **kwargs
    ):
        if precompute_table_approximation == "exact":
            ...
            raise ValueError(
                f"Unknown approximation method: {precompute_table_approximation}"
            )

    def sample_from_cdf(
        self, N: int, pdf_available=False, approximation="exact", **kwargs
    ) -> np.ndarray[float]:
    ...
    def _sample_from_cdf_exact(self, N: int, pdf_available=False):
        ...
        def solve(u, bracket, x0, cdf, pdf):
            ...
            return root_scalar(
                lambda x: cdf(x) - u, bracket=bracket, x0=x0, fprime=pdf
            ).root
