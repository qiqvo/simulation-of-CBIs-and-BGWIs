...
class StableCBI(CriticalCBI):
    ...
    def laplace_transform(
        ...
        return (1 + self.alpha * self.c * np.abs(t) ** self.alpha * time) ** (
            -self.delta
        ) * self._cb.laplace_transform(t, time, z)
    ...
    def sample(
        ...
        s += self._linnik.sample(len(z) * N).reshape(len(z), N) * (
            self.alpha * self.c * time
        ) ** (1 / self.alpha)
        return s
